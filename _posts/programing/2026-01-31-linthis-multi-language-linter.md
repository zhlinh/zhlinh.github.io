...
运行 `linthis fix --help` 或 `linthis hook install --help` 时，会自动检测当前环境中可用的 AI 提供者并在帮助文本末尾显示：

```
AI providers (current environment):
  ✓ claude         Anthropic Claude API (available)
  ✓ claude-cli     Claude CLI (available)
    codebuddy      CodeBuddy API
    openai         OpenAI GPT API
...
| 选项                  | 描述         |
| ------------------- | ---------- |
| `--args <ARGS>`     | 传递给 hook 脚本中 linthis 命令的额外参数，默认 `-c -f` |
| `--event <EVENT>`   | Git hook 事件类型（pre-commit, pre-push, commit-msg），默认 pre-commit |
...
linthis 可以与 AI 编程助手集成，在 AI 辅助开发过程中自动执行代码质量检查。安装后，AI 助手会在修改代码后自动运行 `linthis` 检查，并在提交前修复问题。

**支持的 AI 助手**：

| AI 助手          | 规则文件                                              | 安装策略            |
| --------------- | --------------------------------------------------- | ----------------- |
| Claude Code     | `CLAUDE.md` + `.claude/settings.local.json`          | 追加段落 + Stop Hook |
| Cursor          | `.cursor/rules/linthis.mdc`                          | 独立文件            |
| Windsurf        | `.windsurf/rules/linthis.md`                         | 独立文件            |
| GitHub Copilot  | `.github/copilot-instructions.md`                    | 追加段落            |
| Cline           | `.clinerules/linthis.md`                             | 独立文件            |
| CodeBuddy       | `.codebuddy/rules/linthis.md`                        | 独立文件            |
...
# 卸载 AI 助手集成
linthis hook uninstall --type agent

# 卸载所有 hook（包括 git hook 和 AI 助手集成）
linthis hook uninstall --all -y
```

交互式选择时，已检测到或已安装的 AI 助手会排在列表前面，同时 `--help` 会显示当前环境的检测状态：
...
安装的规则会指导 AI 助手在修改代码后运行 `linthis -i <file> -c` 检查，手动阅读错误并修复代码，提交前运行 `linthis -s -c` 确认通过。
...
# 启用 AI 智能修复（交互式选择 provider）
linthis --fix --ai

# 启用 AI 智能修复（指定 provider）
...
使用 `--ai` 但不指定 `--provider` 时，在终端交互模式下会弹出提供者选择菜单，可用的提供者排在前面：

```
Select AI provider:

  ✓ 1. claude - Anthropic Claude API (available)
  ✓ 2. claude-cli - Claude CLI (available)
    3. codebuddy - CodeBuddy API
    4. codebuddy-cli - CodeBuddy CLI
    5. openai - OpenAI GPT API
    6. local - Local LLM (Ollama)
    7. Cancel
...
| `--provider <NAME>` | 指定 AI 提供者（需要 `--ai`），不指定时交互式选择 |
...
```bash
# 交互式 AI 修复（自动选择 provider）
linthis fix --ai
linthis --fix --ai

# 交互式 AI 修复（指定 provider）
linthis fix --ai --provider claude
linthis --fix --ai --provider claude
